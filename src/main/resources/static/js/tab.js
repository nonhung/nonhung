  ...
  var c = {
            ...
            switch (a) {
              ...
              case"closeAll":
                i._title.children("li[lay-id]").each(function () {
                  ...
                  if (e != -1) {
                    t(this).remove();
                    t('.layui-tab-item[lay-item-id="'+e+'"]', '.layui-tab-content').remove();
                  }
                  t('.layui-tab-item[lay-item-id="-1"]', '.layui-tab-content').addClass('layui-show');
                  t('.layui-this', '.layui-nav-tree').removeClass('layui-this');
                  t('.layui-nav-itemed', '.layui-nav-tree').removeClass('layui-nav-itemed');
                })
            }
            l.click()
          })
    ...
    tabAdd: function (i) {
      var t = this, a = t._config,
        l = (i = i || {id: (new Date).getTime(), title: "新标签页", icon: "fa-file", url: "404.html"}).title, n = i.icon,
        c = i.url, s = i.id;
      if (t.tabExists(s)) t.tabChange(s); else {
        NProgress.start();
        var r = ['<li class="layui-this" id="layui-tab-title-' + i.url + '" lay-id="' + s + '" >'];
        ...
        var o = '<div id="layui-tab-item-' + i.url + '" class="layui-tab-item layui-show" lay-item-id="' + s + '"><iframe src="' + c + '"></iframe></div>';
        t._title.append(r.join("")), t._content.append(o), t.getTab(s).find("i.layui-tab-close").off("click").on("click", function () {
          ...
          a.onSwitch({index: i.index, elem: i.elem, layId: t._title.children("li").eq(i.index).attr("lay-id")})
        })
      }
    },
    tabClose: function (url) {
      var titleElement = document.getElementById('layui-tab-title-' + url);
      var tabElement = document.getElementById('layui-tab-item-' + url);
      if (titleElement && titleElement.length) {
          document.querySelector('.layui-tab-title').removeChild(titleElement);
      }
      if (tabElement && tabElement.length) {
          document.querySelector('.layui-tab-title').removeChild(tabElement);
      }
    }
  };
  i("tab", new n)
