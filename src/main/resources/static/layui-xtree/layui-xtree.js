...
//生产结构
layuiXtree.prototype.dataBind = function (d, parentValue = '') {
  var _this = this;
  if (d.length > 0) {
    for (i in d) {
      var xtree_isend = 'data-xend="1"';
      var xtree_ischecked = '';
      var xtree_isdisabled = '';
      var value = parentValue == '' ? d[i].value : parentValue + ',' + d[i].value;
      _this._domStr += '<div class="layui-xtree-item">';
      if (d[i].data.length > 0)
        ...
        _this._domStr += '<i style="display: inline-block; width: 18px; height: 20px; margin-right: 5px" class="layui-icon"></i>';
        xtree_ischecked = (value == _this._options.checked || d[i].checked) ? ' checked ' : '';
        xtree_isdisabled = d[i].disabled ? ' disabled="disabled" ' : '';
      }
      _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + value + '" title="' + d[i].title + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
      _this.dataBind(d[i].data, value);
      _this._domStr += '</div>';
    }
  }
}
...
layuiXtree.prototype.Rendering = function () {
  ...
  _this._form.on('checkbox(xtreeck' + _this._containerid + ')', function (da) {
    var xtree_chis = document.getElementsByClassName('layui-xtree-checkbox');
    for (var i = 0; i < xtree_chis.length; i++) {
      if (!xtree_chis[i].disabled && da.value !== xtree_chis[i].value) {
        xtree_chis[i].checked = false;
        xtree_chis[i].nextSibling.classList.remove('layui-form-checked');
      }
    }
...
//获取全部选中的末级checkbox对象
layuiXtree.prototype.GetChecked = function (action = '') {
  var _this = this;
  var arr = new Array();
  var arrIndex = 0;
